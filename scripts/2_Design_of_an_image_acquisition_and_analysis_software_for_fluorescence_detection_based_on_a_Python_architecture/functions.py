...
def normalize_image(image, background, scale_factor=0.5):
    ...
    return image_float_corrected

def crop_image(image,background):
    final_background = background
    background = cv2.GaussianBlur(background, (35, 35), 5)
    r, g, b = cv2.split(background)

    _, red_mask = cv2.threshold(r, 35, 255, cv2.THRESH_BINARY)
    ...
    contour_mask = cv2.erode(contour_mask, np.ones((3, 3), np.uint8), iterations=4)
    contour_mask = cv2.dilate(contour_mask, np.ones((3, 3), np.uint8), iterations=2)
    final_background[contour_mask == 255] = 0 

    matrix = cv2.getPerspectiveTransform(src_pts, dst_pts)

    warped_background = cv2.warpPerspective(final_background, matrix, (500, 500))
...
def get_stat_results(input_image, cell_data, grid_mask):
    ...
    treshold = mean_negative_green + 3 * sd_negative_green
    for key in cell_data:
