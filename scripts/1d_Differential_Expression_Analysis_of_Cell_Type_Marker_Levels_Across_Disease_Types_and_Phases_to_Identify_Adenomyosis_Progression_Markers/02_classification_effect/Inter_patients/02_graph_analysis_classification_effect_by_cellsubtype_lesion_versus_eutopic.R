...
AD <- cells_multiplex %>%
  ...
  filter(Classification %in% c("AD_ectopic_adenomyosis", "AD_eutopic")) %>%
  filter(Cells_subgroup!='NA', Cells_subgroup!='Neutrophils')


##Factor --> ordre données dans graph
AD$Classification <- factor(AD$Classification, levels = c('AD_eutopic', 'AD_ectopic_adenomyosis'))
...
cm <- makeContrasts(AD_vs_eutopic = ClassificationAD_ectopic_adenomyosis, levels=mydesign)   
cm
...
###Export results
write.xlsx(result,'../results/02_classification_effect/merged_phases/cell_subgroups_lesion_vs_eutopic_AD.xlsx')


##############################################################################################################################
##############################################################################################################################

##Comparaison Transition_vs_eutopic
...
AD <- cells_multiplex %>%
  group_by(Phase,Image,Classification,Cells_subgroup) %>%
  summarise(Result=sum(Result)) %>% 
  filter(Classification %in% c("Transition_ectopic", "Transition_eutopic")) %>%
  filter(Cells_subgroup!='NA', Cells_subgroup!='Neutrophils')


##Factor --> ordre données dans graph
AD$Classification <- factor(AD$Classification, levels = c("Transition_eutopic", "Transition_ectopic"))
...
cm <- makeContrasts(T_vs_eutopic = ClassificationTransition_ectopic, levels=mydesign)   
cm
fit <- contrasts.fit(fit, cm)
efit <- eBayes(fit,trend=T,robust = T)

result <- topTable(efit, coef="T_vs_eutopic",n=dim(efit)[1],genelist = rownames(AD_omic_data))
result <- result %>% select(ID,logFC,P.Value,adj.P.Val) %>%
  mutate(logFC=round(logFC,2))
...
###Export results
write.xlsx(result,'../results/02_classification_effect/merged_phases/cell_subgroups_lesion_vs_eutopic_Transition.xlsx')
...
AD <- cells_multiplex %>%
  group_by(Phase,Image,Classification,Cells_subgroup) %>%
  summarise(Result=sum(Result)) %>% 
  filter(Classification %in% c("AD_ectopic_adenomyosis", "AD_ectopic_transition")) %>%
  filter(Cells_subgroup!='NA', Cells_subgroup!='Neutrophils')


##Factor --> ordre données dans graph
AD$Classification <- factor(AD$Classification, levels = c('AD_ectopic_transition', 'AD_ectopic_adenomyosis'))
