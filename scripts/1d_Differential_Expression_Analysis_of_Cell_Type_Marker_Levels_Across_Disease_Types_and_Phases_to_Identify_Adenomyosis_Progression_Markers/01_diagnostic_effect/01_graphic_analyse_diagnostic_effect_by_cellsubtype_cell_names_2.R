...
summary_multiplex_cells <- cells_multiplex %>%
  group_by(Image,Type,Cells_subgroup,Phase) %>%
...
##GRAPH
pdf('../results/01_diagnostic_effect/scatterplot_diagnostic_cells_subgroup_phase_separated_log2.pdf',width=6,height = 10)
ggplot(summary_multiplex_cells,mapping = aes(
  ...
  facet_grid(Cells_subgroup~Phase,scales='free_y') + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
dev.off()


pdf('../results/01_diagnostic_effect/scatterplot_diagnostic_cells_subgroup_phase_merged_log2.pdf',width=4,height = 10)
ggplot(summary_multiplex_cells,mapping = aes(
  x = Type,
  y = log2(Result+0.001),
  col=Type)) +
  geom_jitter(width = 0.1,size=0.5,alpha=0.6) +
  stat_summary(geom="point",fun="mean",size=1.5,alpha=0.8)+
  facet_grid(Cells_subgroup~.,scales='free_y') + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
dev.off()
...
summary_wide <- summary_multiplex_cells %>%
  pivot_wider(names_from = Cells_subgroup,values_from = Result)
...
write.xlsx(result,'../results/01_diagnostic_effect/diagnostic_cells_subgroup_phase_merged_AD_versus_control.xlsx')
...
write.xlsx(result,'../results/01_diagnostic_effect/diagnostic_cells_subgroup_phase_merged_T_versus_control.xlsx')
...
write.xlsx(result,'../results/01_diagnostic_effect/diagnostic_cells_subgroup_phase_merged_AD_versus_T.xlsx')
